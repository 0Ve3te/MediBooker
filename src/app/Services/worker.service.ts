...
export class WorkerService {
  ...
  createDay(day: Day): Observable<DayDTO>{
    return this.http.post<DayDTO>(`https://localhost:44379/api/Worker/CreateDay`, this.mapToDayDTO(day));
  }

  loadDays(): Observable<[DayDTO]>{
    return this.http.get<[DayDTO]>(`https://localhost:44379/api/Worker/LoadDay`).pipe(shareReplay());
  }
  ...
  getDoctorSchedule(id: string): Observable<[DayDTO]>{
    return this.http.get<[DayDTO]>(`https://localhost:44379/api/Worekr/LoadDoctorSchedule/${id}`).pipe(shareReplay());
  }

  deleteDay(id: number) {
    return this.http.delete(`https://localhost:44379/api/Worker/DeleteDay/${id}`).pipe(shareReplay());
  }
