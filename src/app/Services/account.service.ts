...
export class AccountService {
  ...
  Login(login: LoginDTO): Observable<TokenInfoDTO> {
    return this.http.post<TokenInfoDTO>(`https://localhost:44379/api/Account/login`, login);
  }
  ...
  firstEdit(profile: User, password: string): Observable<any> {
    ...
    return this.http.put(`https://localhost:44379/api/Account/firstEditUser/${profile.id}/${password}`, formData);
  }

  createDoctor(email: string): Observable<DoctorDTO>{
    const user = new requestCreateUser(email);

    return this.http.post<DoctorDTO>(`https://localhost:44379/api/Moderator/CreateDoctor`, user);
  }

  createWorker(email: string): Observable<DoctorDTO>{
    const user = new requestCreateUser(email);

    return this.http.post<DoctorDTO>(`https://localhost:44379/api/Moderator/CreateWorker`, user);
  }
  ...
  putDoctor(profile: Doctor): Observable<DoctorDTO>{
    ...
    return this.http.put<DoctorDTO>(`https://localhost:44379/api/Account/EditDoctor/${profile.id}`, formData);
  }
  ...
  putWorker(profile: Worker): Observable<WorkerDTO>{
    ...
    return this.http.put<WorkerDTO>(`https://localhost:44379/api/Account/EditWorker/${profile.id}`, formData);
  }
  ...
  putModerator(profile: Worker): Observable<WorkerDTO>{
    ...
    return this.http.put<WorkerDTO>(`https://localhost:44379/api/Moderator/EditModerator/${profile.id}`, formData);
  }

  putPassword(id:string | null | undefined, changePassword: ChangePasswordDTO | undefined): Observable<ChangePasswordDTO>{
    console.log(id)
    console.log(changePassword)
    return this.http.put<ChangePasswordDTO>(`https://localhost:44379/api/Account/ChangePassword/${id}`, changePassword);
  }

  deleteDoctor(id: string){
    return this.http.delete(`https://localhost:44379/api/Moderator/DeleteDoctor/${id}`);
  }

  deleteWorker(id: string){
    return this.http.delete(`https://localhost:44379/api/Moderator/DeleteWorker/${id}`);
  }

  getListDoctors(): Observable<[DoctorDTO]>{
    return this.http.get<[DoctorDTO]>(`https://localhost:44379/api/Account/LoadDoctors`).pipe(shareReplay());
  }

  getListWorkers(): Observable<[WorkerDTO]>{
    return this.http.get<[WorkerDTO]>(`https://localhost:44379/api/Account/LoadWorkers`).pipe(shareReplay());
  }
  ...
  getWorker(id:string): Observable<WorkerDTO>{
    return this.http.get<WorkerDTO>(`https://localhost:44379/api/Account/LoadWorker/${id}`).pipe(shareReplay());
  }

  getModerator(id:string): Observable<WorkerDTO>{
    return this.http.get<WorkerDTO>(`https://localhost:44379/api/Moderator/LoadModerator/${id}`).pipe(shareReplay());
  }
